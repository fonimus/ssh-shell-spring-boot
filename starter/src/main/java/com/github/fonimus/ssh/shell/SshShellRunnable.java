...
public class SshShellRunnable
	...
	public SshShellRunnable(ChannelSession session, Banner shellBanner, PromptProvider promptProvider, Shell shell,
			JLineShellAutoConfiguration.CompleterAdapter completerAdapter, Parser parser, Environment environment, File historyFile,
			org.apache.sshd.server.Environment sshEnv, boolean displayBanner, SshShellCommandFactory sshShellCommandFactory, InputStream is,
			...
			try {
				...
				LineReader reader = LineReaderBuilder.builder()
						...
						.highlighter((reader1, buffer) -> {
							...
							if (best != null) {
								return new AttributedStringBuilder(buffer.length()).append(best, AttributedStyle.BOLD).append(buffer.substring(l))
										.toAttributedString();
							} else {
